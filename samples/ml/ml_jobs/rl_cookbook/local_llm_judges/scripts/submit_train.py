#!/usr/bin/env python3
"""Submit policy RL training using SPCS managed runtime image.
...
def submit_training_job(
    ...
    """Submit policy RL training job using SPCS runtime image."""
    ...
    env_vars = {
        ...
        "PYTHONUNBUFFERED": "1",
        # Local vLLM judge configuration
    ...
    return jobs.submit_directory(
        payload_dir,
        entrypoint=["python", "run_policy_rl.py", "--config", config],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Submit policy RL training (SPCS runtime)")
    parser.add_argument(
    ...
    parser.add_argument("-c", "--config", default="config_policy_rl.yaml")
    ...
    print(f"Submitting policy RL training (SPCS runtime)...")
