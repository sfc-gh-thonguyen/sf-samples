...
def merge_lora_adapter(base_model_path, adapter_path):
    """Merge a LoRA adapter into the base model, return path to merged model.

    Args:
        base_model_path: HuggingFace model name or local path to base model
        adapter_path: Local path to LoRA adapter directory (adapter_model.safetensors + adapter_config.json)
...
async def evaluate_model(model_spec, samples, host):
    ...
    # Parse model spec — supports "backend:model", "backend:model:tp=N", "lora:base:adapter"
    ...
    if backend == "lora":
        # e.g. "lora:Qwen/Qwen3-1.7B:/path/to/adapter"
        segments = model_spec.split(":")
        backend = "lora"
        base_model = segments[1]
        lora_adapter_path = segments[2] if len(segments) > 2 else None
        tp_parts = [s for s in segments[3:] if s.startswith("tp=")]
        if tp_parts:
            tp = int(tp_parts[0].split("=")[1])
        if not lora_adapter_path:
            raise ValueError(f"lora backend requires adapter path: lora:base_model:adapter_path")
    elif ":tp=" in model_spec:
    ...
    if backend == "lora":
        ...
        start_gen_server(merged_path, tp=tp, gpu_ids=gpu_ids)
    ...
    for i, sample in enumerate(samples):
        ...
        try:
            ...
            elif backend in ("hf", "checkpoint", "lora"):
